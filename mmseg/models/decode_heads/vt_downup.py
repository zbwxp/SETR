...
class vt_downup(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 num_ziper_layer=3,
                 **kwargs):
        super(vt_downup, self).__init__(**kwargs)

        down_token_layer = FilterTokenizer(
            in_channels=kwargs['in_channels'],
            token_channels=kwargs['embed_dim'],
            tokens=VT_query//4,
        )

        up_token_layer = FilterTokenizer(
            in_channels=kwargs['embed_dim'],
            token_channels=kwargs['embed_dim'],
            tokens=VT_query,
        )

        self.down = down_token_layer
        self.up = up_token_layer
