...
class vit_seq_head_downup(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 use_attn=False,
                 **kwargs):
        super(vit_seq_head_downup, self).__init__(**kwargs)

        self.pos_style = zipee_pos_style
        self.num_queries_down = ziper_query//4
        hidden_dim = kwargs['embed_dim']
        self.use_attn = use_attn
        self.zip_query_embed = nn.Embedding(self.num_queries_down, hidden_dim)
        ...
        self.expand_query_embed = nn.Embedding(ziper_query, hidden_dim)
    ...
    def forward(self, input):
        ...
        query_embed = self.expand_query_embed.weight
        hs_up, _ = self.expand_transformer(src, mask, query_embed, pos)
        x = hs_up[0]
        if x.dim() == 3:
            if x.shape[1] % 32 != 0:
                x = x[:, 1:]
