...
class vit_seq_clean_all_zero(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 num_heads=12,
                 **kwargs):
        super(vit_seq_clean_all_zero, self).__init__(**kwargs)
        ...
        self.down_decoder = nn.TransformerDecoder(decoder_layer, num_ziper_layer)
        ...
        self.zip_pos_embed = PositionalEncoding(kwargs['embed_dim'], expand_query)
    ...
    def forward(self, x):
        ...
        down_x = self.down_decoder(
            torch.zeros_like(x).transpose(0, 1)[:self.num_queries],
            x.transpose(0, 1))

        up_x = self.up_decoder(
            torch.zeros_like(x).transpose(0, 1),
            down_x)
