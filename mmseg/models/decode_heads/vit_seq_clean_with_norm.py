...
class vit_seq_clean_with_norm(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 num_heads=12,
                 **kwargs):
        super(vit_seq_clean_with_norm, self).__init__(**kwargs)
        ...
        down_norm = nn.LayerNorm(dim)
        up_norm = nn.LayerNorm(dim)

        self.down_decoder = nn.TransformerDecoder(decoder_layer, num_expand_layer, norm=down_norm)
        self.up_decoder = nn.TransformerDecoder(decoder_layer, num_expand_layer, norm=up_norm)
    ...
    def forward(self, x):
        ...
        down_x = self.down_decoder(
            self.zip_query_embed.weight.repeat(bs, 1, 1).transpose(0, 1),
            x.transpose(0, 1))
