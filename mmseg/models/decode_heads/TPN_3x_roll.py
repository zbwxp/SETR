...
class TPN_3x_roll(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 use_norm=False,
                 **kwargs):
        super(TPN_3x_roll, self).__init__(**kwargs)
    ...
    def forward(self, x):
        ...
        bs = x1.size()[0]
        x3 = self.up_decoder_1(
            self.expand_query_embed.weight.repeat(bs, 1, 1).transpose(0, 1),
            x3.transpose(0, 1))
        x2 = self.up_decoder_2(x3, x2.transpose(0, 1))
        x1 = self.up_decoder_3(x2, x1.transpose(0, 1))
