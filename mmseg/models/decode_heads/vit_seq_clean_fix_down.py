...
class vit_seq_clean_fix_down(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 num_heads=12,
                 **kwargs):
        super(vit_seq_clean_fix_down, self).__init__(**kwargs)
        ...
        self.tgt = PositionalEncoding(kwargs['embed_dim'], ziper_query)
    ...
    def forward(self, x):
        ...
        down_x = self.down_decoder(
            self.tgt.pos_table.clone().detach().repeat(bs, 1, 1).transpose(0, 1),
            x.transpose(0, 1))
