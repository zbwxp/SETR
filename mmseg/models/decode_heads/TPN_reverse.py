...
from ..utils.transformer_reverse import TransformerDecoderLayer_reverse
...
class TPN_reverse(VisionTransformerUpHead):
    ...
    def __init__(self,
                 ...
                 use_norm=False,
                 **kwargs):
        super(TPN_reverse, self).__init__(**kwargs)

        dim = kwargs['embed_dim']
        decoder_layer = TransformerDecoderLayer_reverse(d_model=dim, nhead=num_heads, dim_feedforward=dim * 4)
        ...
        self.shrink_query_embed = nn.Embedding(shrink_query, dim)
    ...
    def forward(self, x):
        ...
        x2 = self.down_decoder(
            self.shrink_query_embed.weight.repeat(bs, 1, 1).transpose(0, 1),
            x2.transpose(0, 1))
