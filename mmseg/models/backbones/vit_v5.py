...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., shrink=False):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, shrink=False):
        super().__init__()
    ...
    def forward(self, x, shrink=False):
        if shrink:
            x = self.drop_path(self.attn(self.norm1(x.detach()), shrink))
...
class vit_v5(nn.Module):
    ...
    def __init__(self, model_name='vit_large_patch16_384', img_size=384, patch_size=16, in_chans=3, embed_dim=1024,
        ...
        super(vit_v5, self).__init__(**kwargs)
        ...
        self.scale = (self.embed_dim // num_heads) ** -0.5
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if self.shrink[i]:
                x_ = x.clone().detach()
            x = blk(x, self.shrink[i])
            if i in self.out_indices:
                if i==3 or i==7 or i==11:
                    bs, n_q, ch = x.size()
                    n_k = x_.size()[1]
                    q = x.reshape(bs, n_q, self.num_heads,
                                  ch // self.num_heads).permute(0, 2, 1, 3)
                    k = x_.reshape(bs, n_k, self.num_heads,
                                  ch // self.num_heads).permute(0, 2, 1, 3)
                    attn = (q @ k.transpose(-2, -1)) * self.scale
                    out = attn.sum(dim=1).transpose(-1, -2)
                    if self.shrink[i]:
                        outs.append(out)
                    else:
                        outs.append((out + outs[i-4])/2.0)
                else:
                    outs.append(x)
        return tuple(outs)
